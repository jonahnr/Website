...
function setElementHiddenFromFocus(element, isHidden) {
  ...
  element.querySelectorAll("a, button, input, select, textarea, [tabindex]").forEach((focusable) => {
    if (isHidden) {
      if (focusable.hasAttribute("tabindex")) {
        focusable.dataset.previousTabindex = focusable.getAttribute("tabindex") || "";
      }
      focusable.setAttribute("tabindex", "-1");
      return;
    }
    if (focusable.dataset.previousTabindex !== undefined) {
      const previousTabindex = focusable.dataset.previousTabindex;
      if (previousTabindex) {
...
function setupCarousel(name) {
  ...
  function render() {
    state.items.forEach((item) => {
      ...
      setElementHiddenFromFocus(item, !isActive);
    });
...
function setupInsightFilters() {
  ...
  function renderFilter(category) {
    ...
    insightCards.forEach((card) => {
      ...
      setElementHiddenFromFocus(card, !isVisible);
      if (isVisible) {
        visibleCount += 1;
      }
    });
